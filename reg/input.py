...
class Input:
    def __init__(self):
        ...
        self.events = []  # Store events for key down detection
    ...
    def update(self):
        self.pressed = pygame.key.get_pressed()
        self.mouse = pygame.mouse.get_pressed()
        self.events = pygame.event.get()  # Ensure events are updated

    def get_key_pressed(self, key):
        return self.pressed[key]

    def get_key_down(self, key):
        # Check if the key was just pressed down in the current frame
        return any(event.type == pygame.KEYDOWN and event.key == key for event in self.events)
